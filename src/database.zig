...
const FileBuffer = struct {
    ...
    pub fn init(name: []const u8) FileBuffer {
        const file = zvm_dir.createFile(name, .{ .truncate = false, .read = true, .lock = .exclusive })
...
pub const FileBufferSet = struct {
    ...
    pub fn init(name: []const u8) FileBufferSet {
        const file = zvm_dir.createFile(name, .{ .truncate = false, .read = true })
            catch |e| fatal("{}: cannot create file: {s}/{s}", .{ e, ZVM_STORAGE_PATH, name });
...
// pub var zig_file: std.fs.File = undefined; // @init_on_main
pub var zvm_path: []const u8 = undefined; // @init_on_main
pub var zvm_dir: std.fs.Dir = undefined; // @init_on_main
pub var arena: std.mem.Allocator = undefined; // @init_on_main
                                              

const main = @import("main.zig");

pub fn detect_zvm_installation(env: std.process.EnvMap, arena_allocator: Allocator) void {
...
pub fn nuke() void {
    std.fs.deleteTreeAbsolute(zvm_path) catch |e| fatal("cannot delete directory {s}: {}", .{ zvm_path, e });
}

pub fn init(fetch: bool, client: *std.http.Client) void {
    zvm_dir = std.fs.openDirAbsolute(zvm_path, .{.iterate = true}) catch |e| 
        if (e == error.FileNotFound) blk: {
            if (!main.ask_for_yes("zvm is going to create {s}. You can delete it anytime.", .{ zvm_path })) return;
            std.fs.makeDirAbsolute(zvm_path) catch fatal("cannot create directory \"{s}\"", .{ zvm_path });
            break :blk std.fs.openDirAbsolute(zvm_path, .{.iterate = true}) catch fatal("cannot open directory \"{s}\"", .{ zvm_path });
    ...
    const fresh_index = if (index_file.fetch_if_empty_or_force(
        OFFICIAL_DOWNLOAD ++ "/index.json", client, fetch)) true else |e| fallback: {
